...
public class RentalDB {
	...
	public void endRental(Rental rental) throws Exception {
		...
		try {
			...
			List<Rental> rentList = findRentalsByDateAndAssistiveDeviceId(rental.getAssistiveDeviceInstance().getId(), rental.getStartDate(), rental.getEndDate());
	...
	private Rental buildRentalObject(ResultSet rs) throws SQLException {
		Rental rental = null;
		try {
			rental = new Rental(rs.getInt("id"), rs.getInt("rentalNumber"), rs.getDate("startDate").toLocalDate(), (rs.getDate("endDate") != null ? rs.getDate("endDate").toLocalDate() : null));						
		} catch (SQLException e) {
			e.printStackTrace();
		}
		return rental;
