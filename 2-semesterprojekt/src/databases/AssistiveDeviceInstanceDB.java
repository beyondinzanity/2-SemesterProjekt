...
public class AssistiveDeviceInstanceDB implements IAssistiveDeviceInstanceDB {
	...
	private static final String FIND_BY_DEVICE_ID = "select * from AssistiveDeviceId where id = ?";
	...
	private AssistiveDeviceInstance buildAssistiveDeviceInstanceObject(ResultSet rs) {
		...
		try {
			assistiveDeviceInstance = new AssistiveDeviceInstance(rs.getString("barcode"), rs.getDate("registeredDate"), rs.getString("note"));
		} catch (SQLException s) {
			s.printStackTrace();
		}
